...
data = pd.read_csv("yaw13.csv", header=None, delimiter=',', skiprows=1)
...
def position_callback(timestamp, data, logconf):
    ...
    if log_positions:  # Check if logging is enabled
        ...
        file_path = 'traject13_log6T_Robust_Mellinger_cf2.csv'
        file_exists = os.path.exists(file_path)

        # Write headers only if the file doesn't exist
        with open(file_path, 'a', newline='') as file:
            df = pd.DataFrame({'timestamp': [timestamp], 'Kalman.stateX': [x], 'Kalman.stateY': [y], 'Kalman.stateZ': [z], 'stabilizer.yaw': [yaw]})
