...
import time
import threading
from opencv_viewer2 import start_video_streaming
...
data = pd.read_csv("yaw19.csv", header=None, delimiter=',', skiprows=1)
...
def run_sequence(cf, trajectory_id, duration):
    ...
    time.sleep(3.0)
    #commander.go_to(1.0, 1.5, 0.2, 0, 1.0, relative = False)
    #time.sleep(1.0)
...
def position_callback(timestamp, data, logconf):
    ...
    if log_positions:  # Check if logging is enabled
        ...
        file_path = 'OOI_traject19_log4_Robust_Mellinger_cf2.csv'
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        #thread_video_streaming = threading.Thread(target=start_video_streaming)
        #thread_video_streaming.start()
